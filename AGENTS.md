...
CollaborationMCPRadar는 협업/커뮤니케이션 MCP 후보를 수집하고 repository metadata, capability, risk signal을 DuckDB/HTML 리포트로 정리하는 MCP Radar 저장소입니다.
...
│   └── categories/collaboration_mcp.yaml   # MCP category 정의
...
- `config/categories/collaboration_mcp.yaml`은 MCP 후보 metadata, source contract, activation gate의 기준 파일로 유지한다.
