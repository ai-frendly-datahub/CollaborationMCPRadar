# CollaborationMCPRadar - 협업/커뮤니케이션 MCP 레이더

**🌐 Live Report**: https://ai-frendly-datahub.github.io/CollaborationMCPRadar/


Collaboration & Communication 섹션의 한국 MCP 서버 목록을 수집하고 리스크/활성도를 추적하는 독립 Radar입니다.

## 프로젝트 목표

- **데이터 수집**: RSS 피드 및 API
- **엔티티 분석**: 도메인별 키워드 매칭
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 협업/커뮤니케이션 MCP 동향 시각화
...
2. 실행:
   ```bash
   python main.py --category collaboration_mcp --recent-days 7
   # 리포트: reports/collaboration_mcp_report.html
   ```
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY`는 `collaboration_mcp`로 고정합니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `collaboration_mcp`인지 확인
...
```
CollaborationMCPRadar/
  main.py                 # CLI 엔트리포인트
  requirements.txt        # 의존성
  config/
    config.yaml           # DB/리포트 경로 설정
    categories/
      collaboration_mcp.yaml  # 소스 + 엔티티 정의
  radar/
    collector.py          # 데이터 수집
    analyzer.py           # 엔티티 태깅
...
- Source section: `Collaboration & Communication`
...
- GitHub Pages visualization: `reports/index.html` (valid HTML); https://ai-frendly-datahub.github.io/CollaborationMCPRadar/.
